...
const [todos, setTodos] = useState([
  ...
  return (
    <>
      {!minimmize && (
        <div
          className="relative w-[280px] rounded-xl shadow-sm p-0.5 transition-colors duration-300"
        ...
        >
          ...
          <div className="flex items-center justify-between pl-3 pr-4 p-2 w-[280px]">
            <h3 className="text-[#C4C7B7] font-semibold px-2">Today</h3>
        ...
        "
              >
                {/* RADIO - hidden until hover OR visible always if checked */}
                ...
                {/* TEXT - moves slightly to right on hover */}
               <span
                className={`
          ...
          {!showInput ? (
            <div className="flex items-center gap-1 p-1">
              ...
              <span className="hover:bg-[#2A2C21] p-1 pl-2 pr-2 rounded-lg mr-3 mt-1">
                <img src={templetIcon} className=" w-5 h-5  brightness-200 saturate-0 invert-[75%]  " />
              </span>
            </div>
          ) : (
              <div className="p-2 px-3">
                <input
                  ...
                  className="w-full bg-[#1A1A1A] text-[#C4C7B7] outline-[#4d73ff] rounded-lg p-2 outline-3 resize-none h-12 hover:outline-none overflow-hidden text-ellipsis whitespace-nowrap"
        ...
        <div className=" rounded-lg h-30 w-10 flex items-center justify-center" style={{ backgroundColor: listColor }}>
          <span className="rotate-90 origin-center text-white font-medium  flex items-center justify-between gap-1">
            <span className="hover:bg-[#242528] rounded-md p-1 ml-2">
              <CiMaximize1 onClick={() => setMinimize((prev) => !prev)} className="text-white rotate-135 origin-center" />

            </span>
            Today <span className="text-white ml-4 mr-3 ">{todos.length}</span>
          </span>
