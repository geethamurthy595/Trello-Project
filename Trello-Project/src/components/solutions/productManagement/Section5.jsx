import secimg from "../../../assets/sec5img.webp"

function Section5(){
    return <>
    <div className="h-[530px] pl-[190px] flex gap-80">
            <div>
            <img className="h-[370px] w-[370px]" src={secimg} alt="Signup" />
            </div>
            <div>
                <h1 className="h-[96px] w-[442px] text-[36px] font-medium">Power-Up Your Launches</h1><br />
                <p className="text-[20px] font-normal h-[180px] w-[420px]">Spend more time launching products and less time doing administrative heavy-lifting. With Power-Ups, you can connect your favorite digital tools like Google Drive and Slack with Trello, and features like Calendar allow your teams to get a bird’s eye view of product roadmaps.</p><br />
                <p className="underline text-[20px] font-normal text-blue-600">Explore 150+ Power-Ups</p>
            </div>
        </div>
    </>
}
export default Section5